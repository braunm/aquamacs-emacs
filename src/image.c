...
#include <ImageIO/CGImageSource.h> // for CGImageSourceRef
#include <ImageIO/CGImageProperties.h> // for kCGImageProperty...
...
static Lisp_Object mac_preprocess_image_for_2x_file (struct frame *, struct image *, Lisp_Object, int *);
static Lisp_Object mac_preprocess_image_for_2x_data (struct frame *, struct image *, Lisp_Object, bool);
static CFStringRef mac_create_type_identifier_for_image_spec (Lisp_Object);
static void mac_postprocess_image_for_2x (struct image *);
static void mac_cg_image_source_get_pixel_size (CGImageSourceRef, size_t,
						int *, int *);
...
Lisp_Object cfstring_to_lisp_nodecode (CFStringRef);
Lisp_Object cfstring_to_lisp (CFStringRef);
CFStringRef cfstring_create_with_string_noencode (Lisp_Object);
CFStringRef cfstring_create_with_string (Lisp_Object);
Lisp_Object cfdate_to_lisp (CFDateRef);
Lisp_Object cfdata_to_lisp (CFDataRef);
Lisp_Object cfboolean_to_lisp (CFBooleanRef);
static void cfdictionary_add_to_list (const void *, const void *, void *);
static void cfdictionary_puthash (const void *, const void *, void *);
Lisp_Object cfnumber_to_lisp (CFNumberRef);
  ...
  if (has_alpha_p || fmod (rotation, 90) != 0)
    {
	  ...
	  || !x_defined_color (f, SSDATA (specified_bg), &color, 0))
	{
	  if (default_bg == NULL)
	    {
...
mac_create_type_identifier_for_image_spec (Lisp_Object spec)
{
  ...
  //tag_class = UTTagClassFilenameExtension;
  else
    {
         tag_class = kUTTagClassMIMEType;
	 //tag_class = UTTagClassMIMEType;
